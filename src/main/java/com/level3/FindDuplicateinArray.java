...
public class FindDuplicateinArray {
    ...
    public int repeatedNumber(final List<Integer> a) {
        int sz = a.size()-1;
        double sq  = Math.sqrt(sz);
        int size = (int) Math.ceil(sq);
        int floor = (int) Math.floor(sq);
        int[] countArray = new int[size+1];

        for(int i=0;i<a.size();i++){
            countArray[(a.get(i)-1)/floor]++;
        }

        int i;
        for( i=0;i<countArray.length;i++){
            if((floor!=size && countArray[i]>=size) || (floor==size && countArray[i]>=size+1)){
                //c[i] has repeating elements.
                break;
            }
        }

        int lower = 1 + (int) Math.sqrt(sz)*(i);
        int upper = (int) Math.sqrt(sz)*(i+1);

        Arrays.fill(countArray,0);
        for(i=0;i<a.size();i++){
            if(a.get(i)>=lower && a.get(i)<=upper){
                if(countArray[a.get(i)-lower]>0){
                    return a.get(i);
                }
                countArray[a.get(i)-lower]++;
            }
        }
